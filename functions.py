...
def create_job(db:Session, title, link, company, due, user_id, user, status: models.Status ): #create new, use post?
    new_job = models.JobApplications( title = title, company = company, due_date = due, link = link, status = status)
    db.add(new_job)
    db.commit()
    db.refresh(new_job)
    return new_job
